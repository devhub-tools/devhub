defmodule Devhub.Portal.Charts.BugsFixed do
  @moduledoc false
  ...
  @impl true
  def title, do: "Bugs fixed"

  @impl true
  def tooltip, do: "Total number of tickets marked completed that have label type of bug."
  ...
  def render_data_table(assigns) do
    ~H"""
    <.table id="bugs-fixed-data" rows={@data}>
      ...
      <:col :let={i} label="Completed at">
        <div class="text-nowrap">
          <format-date date={i.completed_at}></format-date>
        </div>
      </:col>
  ...
  defp core_query(organization_id, opts) do
    ...
    from i in Issue,
      ...
      on: l.type == :bug,
      as: :label,
      where: not is_nil(i.completed_at),
